...
def test_integration_json_ref_dict():
    """Test the json ref"""
    with open(
        os.path.join("src", "tests", "fixtures", "openapi", "nextcloud-v27.json"),
        "r",
        encoding="UTF-8",
    ) as f:
        ...
        assert_that(
            True,
            equal_to(
                json["paths"]["/ocs/v2.php/core/getapppassword"]["get"]["responses"][
                    "200"
                ]["content"]["application/json"]["schema"]["properties"]["ocs"][
                    "properties"
                ][
                    "meta"
                ][  # "meta" is the dereferenced key
                    "properties"
                ][
                    "itemsperpage"
                ][
