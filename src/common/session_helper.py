...
class SessionHelper:
    @staticmethod
    def session_add(
        model: DeclarativeBase, session: scoped_session, is_test: bool
    ) -> bool:
        if is_test:
            test_output = (
                get_src_dir()
            ...
            primary_key_0 = str(
                getattr(model, inspect(model).mapper.primary_key[0].name)
            )
            filename = (
                f"{str(test_output)}{os.sep}{table_name}{os.sep}{primary_key_0}.json"
            )
